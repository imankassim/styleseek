...
| EXP20 | LLM query parser | Optional parser comparison | Not started (scope decision) | Optional per architecture §14; the deterministic parser (Stage 9, `backend/app/query_understanding.py`) already met the "safe structured constraints" exit outcome. Explicitly not pursued (2026-09-08, user decision) — unlike every other optional extension in this project, this one needs a paid third-party LLM API and credentials, not just local compute. A deliberate scope boundary, not an oversight. |
...
- 2026-09-08 — Stage 14 (bounded context, G9) shipped as direct engineering work rather than an
